= Repository Configuration

Configure necessary software repositories and update installed packages.

[.procedure]
ifdef::rhel-7[]
include::../rhel-7/listitem_repo_config_rhel.adoc[]
ifdef::icehouse[]
include::../rhel-7/icehouse/listitem_repo_config_openstack.adoc[]
endif::icehouse[]
ifdef::juno-rdo[]
include::../rhel-7/juno-rdo/listitem_repo_config_openstack.adoc[]
endif::juno-rdo[]
include::../rhel-7/listitem_repo_config_datastax.adoc[]
ifdef::icehouse[]
include::../rhel-7/icehouse/listitem_repo_config_midonet.adoc[]
endif::icehouse[]
ifdef::juno-rdo[]
include::../rhel-7/juno-rdo/listitem_repo_config_midonet.adoc[]
endif::juno-rdo[]
include::../rhel-7/listitem_install_updates.adoc[]
endif::rhel-7[]

ifdef::ubuntu-1404[]
include::../ubuntu-1404/listitem_repo_config_ubuntu.adoc[]
ifdef::juno[]
include::../ubuntu-1404/juno/listitem_repo_config_ubuntu_cloudarchive.adoc[]
endif::juno[]
include::../ubuntu-1404/listitem_repo_config_datastax.adoc[]
include::../ubuntu-1404/shared/listitem_repo_config_midonet.adoc[]
include::../ubuntu-1404/listitem_install_updates.adoc[]
endif::ubuntu-1404[]

include::../common/listitem_reboot_system.adoc[]
