...
including tenant routers and Edge Routers.
...
to the link-local address, which, in this example, is peered with an Edge
Router.
...
peered with an Edge Router. For traffic that matches the source IP
...
tenant router is peered with port0 on router1, which is the Edge Router.
